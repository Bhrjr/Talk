...
#          @partner = "" if @pid == xx #特定性格の場合の特殊二人称
    ...
    unless $game_party.in_partybattle? or $msg.t_enemy.love > 0
      ...
      #▼会話対象が主人公でない場合
        ...
        @target = "dear" if @doppel #同属仲間への二人称
        #会話対象がパートナーなので、相方は自動的に主人公となる
        @partner = "dear" #主人公への二人称
      end
      ...
      if $game_switches[97] == true
        ...
        if $msg.coop_leader != $msg.t_enemy
          ...
          @cp_leader = "dear" if @doppel_cp #同属相方への二人称
          @cp_leader = "ma'am" if $data_SDB[$msg.coop_leader.class_id].name == "ダークエンジェル" #連携相手がダークエンジェル
        end
      end
    #=====================================================================
      ...
      if $msg.t_target == $game_actors[101]
        ...
        if $msg.t_partner != nil
          if @companion_pt == true#相手の事を知っている場合の二人称
            ...
            @partner = "dear" if $msg.age2 < 0 #年上仲間への二人称
            @partner = "dear" if @doppel_pt #同属仲間への二人称
            @partner = "ma'am" if $data_SDB[$msg.t_partner.class_id].name == "ダークエンジェル" #相手がダークエンジェル
          end
        end
      #▼会話対象が主人公でない場合
      elsif $msg.t_target != $game_actors[101]
        if @companion == true#相手の事を知っている場合の二人称
          ...
          @target = "dear" if $msg.age < 0 #年上仲間への二人称
          @target = "dear" if @doppel #同属仲間への二人称
          @target = "ma'am" if $data_SDB[$msg.t_target.class_id].name == "ダークエンジェル" #相手がダークエンジェル
        end
      end
      #▼連携が発生している場合、別途呼称を設定する
      if $game_switches[97] == true
        ...
        if $msg.coop_leader != $msg.t_enemy
          if @companion_cp == true
            ...
            @cp_leader = "dear" if @doppel_cp #同属仲間への二人称
            @cp_leader = "ma'am" if $data_SDB[$msg.coop_leader.class_id].name == "ダークエンジェル" #連携相手がダークエンジェル
          end
        end
      end
