...
#          @partner = "" if @pid == xx #特定性格の場合の特殊二人称
    ...
    unless $game_party.in_partybattle? or $msg.t_enemy.love > 0
      ...
      if $msg.t_target == $game_actors[101]
        @target = "おにーさん" #主人公への二人称
        #パートナーが存在する場合別途設定
        if $msg.t_partner != nil
          @partner = "あなた" #仲間への二人称
          @partner = "あなた" if $msg.age2 > 0 #年下仲間への二人称
          @partner = "おねーさん" if $msg.age2 < 0 #年上仲間への二人称
      ...
      #▼会話対象が主人公でない場合
        ...
        @target = "おねーさん" if $msg.age < 0 #年上仲間への二人称
        @target = "あなた" if @doppel #同属仲間への二人称
        #会話対象がパートナーなので、相方は自動的に主人公となる
        @partner = "おにーさん" #主人公への二人称
      end
      ...
      if $game_switches[97] == true
        ...
        if $msg.coop_leader != $msg.t_enemy
          ...
          @cp_leader = "おねーさん" if $msg.age3 < 0 #年上相方への二人称
      ...
      if $msg.t_target == $game_actors[101]
        ...
        if $msg.t_partner != nil
          if @companion_pt == true#相手の事を知っている場合の二人称
            @partner = "#{$msg.short_name($msg.t_partner)}ちゃん" #仲間への二人称
            @partner = "#{$msg.short_name($msg.t_partner)}ちゃん" if $msg.age2 > 0 #年下仲間への二人称
            @partner = "#{$msg.short_name($msg.t_partner)}ねーさま" if $msg.age2 < 0 #年上仲間への二人称
            @partner = "#{$msg.short_name($msg.t_partner)}ちゃん" if @doppel_pt #同属仲間への二人称
          else
            @partner = "あなた" #仲間への二人称
            @partner = "あなた" if $msg.age2 > 0 #年下仲間への二人称
            @partner = "ねーさま" if $msg.age2 < 0 #年上仲間への二人称
            @partner = "あなた" if @doppel_pt #同属仲間への二人称
          end
        end
      #▼会話対象が主人公でない場合
        ...
        if @companion == true#相手の事を知っている場合の二人称
          @target = "#{$msg.short_name($msg.t_target)}ちゃん" #仲間への二人称
          @target = "#{$msg.short_name($msg.t_target)}ちゃん" if $msg.age > 0 #年下仲間への二人称
          @target = "#{$msg.short_name($msg.t_target)}ねーさま" if $msg.age < 0 #年上仲間への二人称
          @target = "#{$msg.short_name($msg.t_target)}ちゃん" if @doppel #同属仲間への二人称
        else
          @target = "あなた" #仲間への二人称
          @target = "あなた" if $msg.age > 0 #年下仲間への二人称
          @target = "ねーさま" if $msg.age < 0 #年上仲間への二人称
          @target = "あなた" if @doppel #同属仲間への二人称
        end
      end
      #▼連携が発生している場合、別途呼称を設定する
