    ...
    unless $game_party.in_partybattle? or $msg.t_enemy.love > 0
      ...
      if $msg.t_target == $game_actors[101]
        ...
        if $msg.t_partner != nil
          @partner = "お前" #仲間への二人称
          @partner = "お前" if $msg.age2 > 0 #年下仲間への二人称
          @partner = "お前" if $msg.age2 < 0 #年上仲間への二人称
          @partner = "お前" if @doppel_pt #同属仲間への二人称
        end
      #▼会話対象が主人公でない場合
      elsif $msg.t_target != $game_actors[101]
        @target = "お前" #仲間への二人称
        @target = "お前" if $msg.age > 0 #年下仲間への二人称
        @target = "お前" if $msg.age < 0 #年上仲間への二人称
        @target = "お前" if @doppel #同属仲間への二人称
      ...
      if $game_switches[97] == true
        ...
        if $msg.coop_leader != $msg.t_enemy
          @cp_leader = "お前" #相方への二人称
          @cp_leader = "お前" if $msg.age3 > 0 #年下相方への二人称
          @cp_leader = "お前" if $msg.age3 < 0 #年上相方への二人称
          @cp_leader = "お前" if @doppel_cp #同属相方への二人称
          @cp_leader = "おチビ" if $data_SDB[$msg.coop_leader.class_id].name == "インプ" #連携相手がインプ
          @cp_leader = "姐さん" if $data_SDB[$msg.coop_leader.class_id].name == "デーモン" #連携相手がデーモン
        end
      end
    #=====================================================================
      ...
      if $msg.t_target == $game_actors[101]
        ...
        if $msg.t_partner != nil
          if @companion_pt == true#相手の事を知っている場合の二人称
            ...
            @partner = "#{$msg.short_name($msg.t_partner)}姐さん" if $msg.age2 < 0 #年上仲間への二人称
            @partner = "#{$msg.short_name($msg.t_partner)}" if @doppel_pt #同属仲間への二人称
          else
            @partner = "お前" #仲間への二人称
            @partner = "お前" if $msg.age2 > 0 #年下仲間への二人称
            @partner = "姐さん" if $msg.age2 < 0 #年上仲間への二人称
            @partner = "お前" if @doppel_pt #同属仲間への二人称
            @partner = "おチビ" if $data_SDB[$msg.t_partner.class_id].name == "インプ" #相手がインプ
          end
        end
      #▼会話対象が主人公でない場合
      elsif $msg.t_target != $game_actors[101]
        if @companion == true#相手の事を知っている場合の二人称
          ...
          @target = "#{$msg.short_name($msg.t_target)}姐さん" if $msg.age < 0 #年上仲間への二人称
          @target = "#{$msg.short_name($msg.t_target)}" if @doppel #同属仲間への二人称
        else
          @target = "お前" #仲間への二人称
          @target = "お前" if $msg.age > 0 #年下仲間への二人称
          @target = "姐さん" if $msg.age < 0 #年上仲間への二人称
          @target = "お前" if @doppel #同属仲間への二人称
          @target = "おチビ" if $data_SDB[$msg.t_target.class_id].name == "インプ" #相手がインプ
        end
      end
      #▼連携が発生している場合、別途呼称を設定する
      if $game_switches[97] == true
        ...
        if $msg.coop_leader != $msg.t_enemy
          if @companion_cp == true
            ...
            @cp_leader = "#{$msg.short_name($msg.coop_leader)}姐さん" if $msg.age3 < 0 #年上相方への二人称
            @cp_leader = "#{$msg.short_name($msg.coop_leader)}" if @doppel_cp #同属相方への二人称
          else
            @cp_leader = "お前" #相方への二人称
            @cp_leader = "お前" if $msg.age3 > 0 #年下仲間への二人称
            @cp_leader = "姐さん" if $msg.age3 < 0 #年上仲間への二人称
            @cp_leader = "お前" if @doppel_cp #同属仲間への二人称
            @cp_leader = "おチビ" if $data_SDB[$msg.coop_leader.class_id].name == "インプ" #連携相手がインプ
          end
        end
      end
