    ...
    unless $game_party.in_partybattle? or $msg.t_enemy.love > 0
      ...
      if $msg.t_target == $game_actors[101]
        ...
        if $msg.t_partner != nil
          @target = "貴方" #仲間への二人称
          @target = "貴方" if $msg.age2 > 0 #年下仲間への二人称
          @target = "貴方" if $msg.age2 < 0 #年上仲間への二人称
          @target = "貴方" if @doppel_pt #同属仲間への二人称
        end
      #▼会話対象が主人公でない場合
...
#            @partner = "#{$msg.short_name($msg.t_partner)}" if $data_SDB[$msg.t_partner.class_id].name == "夢魔" #相手が夢魔
            ...
            @target = "貴方" if $msg.age2 < 0 #年上仲間への二人称
            @target = "貴方" if @doppel_pt #同属仲間への二人称
#            @target = "貴女" if $data_SDB[$msg.t_partner.class_id].name == "夢魔" #相手が夢魔
          end
        end
      #▼会話対象が主人公でない場合
      elsif $msg.t_target != $game_actors[101]
...
#          @target = "#{$msg.short_name($msg.t_target)}" if $data_SDB[$msg.t_target.class_id].name == "夢魔" #相手が夢魔
          ...
          @target = "貴方" #仲間への二人称
          @target = "貴方" if $msg.age > 0 #年下仲間への二人称
          @target = "貴方" if $msg.age < 0 #年上仲間への二人称
          @target = "貴方" if @doppel #同属仲間への二人称
#          @target = "貴方" if $data_SDB[$msg.t_target.class_id].name == "夢魔" #相手が夢魔
        end
      end
      #▼連携が発生している場合、別途呼称を設定する
      if $game_switches[97] == true
...
#            @cp_leader  = "貴女" if $data_SDB[$msg.coop_leader.class_id].name == "夢魔" #連携相手が夢魔
          end
        end
      end
    end
