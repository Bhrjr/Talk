...
#######################################################################################################################
 ◆更新履歴
 2016,07/11     二人称を対ロウラット「お兄さん」対年上相方「姉さん」に
 2016,08/12     口上にあわせ@partnerを「お兄さん」「あの子orお姉さん」に
#######################################################################################################################
=end

class MsgLilim_A < MsgBase
  #--------------------------------------------------------------------------
  # ● 性格別の一人称・二人称設定
  #--------------------------------------------------------------------------
  def callname
    ...
    unless $game_party.in_partybattle? or $msg.t_enemy.love > 0
      ...
      if $msg.t_target == $game_actors[101]
        ...
        if $msg.t_partner != nil
          @partner = "あの子" #仲間への二人称
          @partner = "あの子" if $msg.age2 > 0 #年下仲間への二人称
          @partner = "お姉さん" if $msg.age2 < 0 #年上仲間への二人称
          @partner = "あの子" if @doppel_pt #同属仲間への二人称
        end
      #▼会話対象が主人公でない場合
