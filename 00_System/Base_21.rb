    ...
    unless $game_party.in_partybattle? or $msg.t_enemy.love > 0
      ...
      if $msg.t_target == $game_actors[101]
        ...
        if $msg.t_partner != nil
          @partner = "Buddy" #仲間への二人称
          @partner = "L'il buddy" if $msg.age2 > 0 #年下仲間への二人称
          @partner = "Big sis" if $msg.age2 < 0 #年上仲間への二人称
          @partner = "Buddy" if @doppel_pt #同属仲間への二人称
        end
      #▼会話対象が主人公でない場合
      elsif $msg.t_target != $game_actors[101]
        @target = "Buddy" #仲間への二人称
        @target = "L'il buddy" if $msg.age > 0 #年下仲間への二人称
        @target = "Big Sis" if $msg.age < 0 #年上仲間への二人称
        @target = "Buddy" if @doppel #同属仲間への二人称
      ...
      if $game_switches[97] == true
        ...
        if $msg.coop_leader != $msg.t_enemy
          @cp_leader = "Buddy" #相方への二人称
          @cp_leader = "L'il buddy" if $msg.age3 > 0 #年下相方への二人称
          @cp_leader = "Big sis" if $msg.age3 < 0 #年上相方への二人称
          @cp_leader = "Buddy" if @doppel_cp #同属相方への二人称
        end
      end
    #=====================================================================
    #★対味方戦の場合、パートナー夢魔の呼称のみを決める
      ...
      if $msg.t_target == $game_actors[101]
        ...
        if $msg.t_partner != nil
          if @companion_pt == true#相手の事を知っている場合の二人称
            ...
            @partner = "L'il #{$msg.short_name($msg.t_partner)}" if $msg.age2 > 0 #年下仲間への二人称
      ...
      #▼会話対象が主人公でない場合
        ...
        if @companion == true#相手の事を知っている場合の二人称
          ...
          @target = "L'il #{$msg.short_name($msg.t_target)}" if $msg.age > 0 #年下仲間への二人称
      ...
      if $game_switches[97] == true
        ...
        if $msg.coop_leader != $msg.t_enemy
          if @companion_cp == true
            ...
            @cp_leader = "L'il #{$msg.short_name($msg.coop_leader)}" if $msg.age3 > 0 #年下相方への二人称
            ...
            @cp_leader = "Buddy" #相方への二人称
            @cp_leader = "L'il buddy" if $msg.age3 > 0 #年下仲間への二人称
            @cp_leader = "Big sis" if $msg.age3 < 0 #年上仲間への二人称
            @cp_leader = "Buddy" if @doppel_cp #同属仲間への二人称
          end
        end
      end
    end
