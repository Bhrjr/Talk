...
#          @partner = "" if @pid == xx #特定性格の場合の特殊二人称
    ...
    unless $game_party.in_partybattle? or $msg.t_enemy.love > 0
      ...
      if $msg.t_target == $game_actors[101]
        @target = "Master" #主人公への二人称
        #パートナーが存在する場合別途設定
        if $msg.t_partner != nil
          @partner = "Master" #仲間への二人称
          @partner = "Young master" if $msg.age2 > 0 #年下仲間への二人称
          @partner = "Master" if $msg.age2 < 0 #年上仲間への二人称
          @partner = "Lady" if @doppel_pt #同属仲間への二人称
        end
      #▼会話対象が主人公でない場合
      elsif $msg.t_target != $game_actors[101]
        @target = "Master" #仲間への二人称
        @target = "Young master" if $msg.age > 0 #年下仲間への二人称
        @target = "Master" if $msg.age < 0 #年上仲間への二人称
        @target = "Lady" if @doppel #同属仲間への二人称
        #会話対象がパートナーなので、相方は自動的に主人公となる
        @partner = "Master" #主人公への二人称
      end
      #▼連携が発生している場合、別途呼称を設定する
      if $game_switches[97] == true
        #会話の仕掛け手が自分で無い場合、呼称を設定する
        if $msg.coop_leader != $msg.t_enemy
          @cp_leader = "Master" #相方への二人称
          @cp_leader = "Young master" if $msg.age3 > 0 #年下相方への二人称
          @cp_leader = "Master" if $msg.age3 < 0 #年上相方への二人称
    ...
    #★対味方戦の場合、パートナー夢魔の呼称のみを決める
      ...
      if $msg.t_target == $game_actors[101]
        ...
        if $msg.t_partner != nil
          if @companion_pt == true#相手の事を知っている場合の二人称
            @partner = "Sister #{$msg.short_name($msg.t_partner)}" #仲間への二人称
            @partner = "#{$msg.short_name($msg.t_partner)}" if $msg.age2 > 0 #年下仲間への二人称
            @partner = "Sister #{$msg.short_name($msg.t_partner)}" if $msg.age2 < 0 #年上仲間への二人称
      ...
      #▼会話対象が主人公でない場合
        ...
        if @companion == true#相手の事を知っている場合の二人称
          @target = "Sister #{$msg.short_name($msg.t_target)}" #仲間への二人称
          @target = "#{$msg.short_name($msg.t_target)}" if $msg.age > 0 #年下仲間への二人称
          @target = "Sister #{$msg.short_name($msg.t_target)}" if $msg.age < 0 #年上仲間への二人称
      ...
      if $game_switches[97] == true
        ...
        if $msg.coop_leader != $msg.t_enemy
          if @companion_cp == true
            @cp_leader = "Sister #{$msg.short_name($msg.coop_leader)}" #相方への二人称
            @cp_leader = "#{$msg.short_name($msg.coop_leader)}" if $msg.age3 > 0 #年下相方への二人称
            @cp_leader = "Sister #{$msg.short_name($msg.coop_leader)}" if $msg.age3 < 0 #年上相方への二人称
