    ...
    unless $game_party.in_partybattle? or $msg.t_enemy.love > 0
      ...
      if $msg.t_target == $game_actors[101]
        ...
        if $msg.t_partner != nil
          @partner = "貴女" #仲間への二人称
          @partner = "お嬢さん" if $msg.age2 > 0 #年下仲間への二人称
          @partner = "貴女" if $msg.age2 < 0 #年上仲間への二人称
          @partner = "貴女" if @doppel_pt #同属仲間への二人称
          @partner = "ネイジュレンジ様" if $data_SDB[$msg.t_partner.class_id].name == "ネイジュレンジ" #相手がネイジュレンジ
        end
      #▼会話対象が主人公でない場合
      elsif $msg.t_target != $game_actors[101]
        @target = "貴女" #仲間への二人称
        @target = "お嬢さん" if $msg.age > 0 #年下仲間への二人称
        @target = "貴女" if $msg.age < 0 #年上仲間への二人称
        @target = "貴女" if @doppel #同属仲間への二人称
        @target = "ネイジュレンジ様" if $data_SDB[$msg.t_target.class_id].name == "ネイジュレンジ" #相手がネイジュレンジ
      ...
      if $game_switches[97] == true
        ...
        if $msg.coop_leader != $msg.t_enemy
          ...
          @cp_leader = "お嬢さん" if $msg.age3 > 0 #年下相方への二人称
    ...
    #★対味方戦の場合、パートナー夢魔の呼称のみを決める
      ...
      if $msg.t_target == $game_actors[101]
        ...
        if $msg.t_partner != nil
          if @companion_pt == true#相手の事を知っている場合の二人称
            ...
            @partner = "#{$msg.short_name($msg.t_partner)}さん" if $msg.age2 > 0 #年下仲間への二人称
            ...
            @partner = "ネイ様" if $data_SDB[$msg.t_partner.class_id].name == "ネイジュレンジ" #相手がネイジュレンジ
          else
            @partner = "貴女" #仲間への二人称
            @partner = "お嬢さん" if $msg.age2 > 0 #年下仲間への二人称
            @partner = "貴女" if $msg.age2 < 0 #年上仲間への二人称
      ...
      #▼会話対象が主人公でない場合
        ...
        if @companion == true#相手の事を知っている場合の二人称
          ...
          @target = "#{$msg.short_name($msg.t_target)}さん" if $msg.age > 0 #年下仲間への二人称
          @target = "#{$msg.short_name($msg.t_target)}さん" if $msg.age < 0 #年上仲間への二人称
          @target = "#{$msg.short_name($msg.t_target)}さん" if @doppel #同属仲間への二人称
          @target = "ネイ様" if $data_SDB[$msg.t_target.class_id].name == "ネイジュレンジ" #相手がネイジュレンジ
        else
          @target = "貴女" #仲間への二人称
          @target = "お嬢さん" if $msg.age > 0 #年下仲間への二人称
          @target = "貴女" if $msg.age < 0 #年上仲間への二人称
      ...
      if $game_switches[97] == true
        ...
        if $msg.coop_leader != $msg.t_enemy
          if @companion_cp == true
            ...
            @cp_leader = "#{$msg.short_name($msg.coop_leader)}さん" if $msg.age3 > 0 #年下相方への二人称
            ...
            @cp_leader = "ネイ様" if $data_SDB[$msg.coop_leader.class_id].name == "ネイジュレンジ" #相手がネイジュレンジ
          else
            @cp_leader = "貴女" #相方への二人称
            @cp_leader = "お嬢さん" if $msg.age3 > 0 #年下仲間への二人称
            @cp_leader = "貴女" if $msg.age3 < 0 #年上仲間への二人称
            @cp_leader = "貴女" if @doppel_cp #同属仲間への二人称
            @cp_leader = "ネイジュレンジ様" if $data_SDB[$msg.coop_leader.class_id].name == "ネイジュレンジ" #相手がネイジュレンジ
          end
        end
      end
    end
